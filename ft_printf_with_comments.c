...
int	ft_printf(const char *format, ...) {
	va_list	ap;
	va_start(ap, format);
	while (*format) { //идем по строке пока не закончится
		if (*format == '%') { // если встречаем процент, то...
			format++; //...смотрим какой спецификатор стоит после него
			if (*format == 's') { //если спецификатора s
				int	len = 0;
			...
			else if (*format == 'd') { //если спецификатор d
				int d = va_arg(ap, int);
			...
			else if (*format == 'x') { // если спецификатор х
